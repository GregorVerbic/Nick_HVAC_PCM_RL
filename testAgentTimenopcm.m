...
PV = reshape(PV,[],1);
%%%
%PV = PV*1.5; %For increased PV agent
%%%
tin = tout(1);
TIN = [];
ACTION = [];
...
for k = 1:(24*numDays)
    ...
    TIN = [TIN; tin];
    %%%
    action = evaluatePolicy_nopcm_0_67([tin;time]); %This is the policy of the agent
    %%%
    ...
    tin = newInternalTempnopcm(tout(k), tin, action,k);
    hourUse = action + ld(k) - PV(k);
    PVgen = PVgen + PV(k);
    %%%
    price = costTOU(time);
    %price = costFlat;
